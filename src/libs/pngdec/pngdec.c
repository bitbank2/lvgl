...
PNG_STATIC int32_t readMem(PNGFILE * pFile, uint8_t * pBuf, int32_t iLen);
...
{
    if(!pPNG) return PNG_INVALID_PARAMETER;
...
PNG_STATIC int DecodePNG(PNGIMAGE * pPage, void * pUser, int iOptions)
{
    ...
    state = (struct inflate_state *)pPage->ucZLIB;
    d_stream.state = (struct internal_state FAR *)state;
    state->window = &pPage->ucZLIB[sizeof(state[0])]; // point to 32k dictionary buffer
        ...
        while(!bDone) {
            ...
            switch(iMarker) {
                ...
                case 0x49444154: //'IDAT' image data block
                    while(iLen) {
                        if(iOffset >= iBytesRead) {
                            ...
                            iBytesRead = (*pPage->pfnRead)(&pPage->PNGFile, pPage->ucFileBuf,
                                                           (iLen > PNG_FILE_BUF_SIZE) ? PNG_FILE_BUF_SIZE : iLen);
                            iFileOffset += iBytesRead;
                            iOffset = 0;
                        }
                        else {
                ...
                case 0x74455874: { //'tEXt'
                        char szTemp[256];
                        char * pDest = NULL;
                        memcpy(szTemp, &s[iOffset],
                               80); // get the label length (Title, Author, Description, Copyright, Creation Time, Software, Disclaimer, Warning, Source, Comment)
